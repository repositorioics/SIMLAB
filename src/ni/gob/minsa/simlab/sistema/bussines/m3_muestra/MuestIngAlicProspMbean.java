...
@ManagedBean(name="muestIngAlicProspMbean")
@ViewScoped

public class MuestIngAlicProspMbean extends GenericMbean implements Serializable{
			...
			if(itemTypeAlicSelected.length>0){
				for (String itemAlic : itemTypeAlicSelected) {
					...
					//if(SimlabNumberUtil.isNumber(simlabStringUtils.cutToLenght(itemAlic.trim(), 0, 0+1))){
						...
						if(itemAlic.matches(getSufixAlic));
							{
	...
	public boolean validateFieldsToAdd() throws SimlabAppException{	
		...
		if(this.getStudy().matches("Muestreo Anual")||this.getStudy().matches("Muestreo Anual 2017")) this.tipo="muestreoanual";
...
//			this.removeObjectRegAlicFromList();
	...
	public void regAlic(Session session, String codeAlic, Integer posBox, Integer codBox, Integer codFreezer, 
		...
		try {
			...
			throw SimlabAppException.generateExceptionByInsert(MuestIngAlicProspMbean.class, exception);
		}
}
